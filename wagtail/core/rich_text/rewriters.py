...
class TagRewriter:
    def __init__(self, rules=None, bulk_rules=None):
        self.rules = rules or {}
    ...
    def __call__(self, html):
        ...
        for match, replacement in zip(
            chain(*matches_by_tag_type.values()),
            chain(*replacements)
        ):
            html = (
        ...
        if not rule:
            try:
                ...
                if tag_type in ['email', 'external', 'anchor']:
                    ...
                    # <a href="https://wagtail.io">, so we return an empty list
                    # here so that no tag replacements are made.
                    return []
            else:
                rule = self.convert_rule_to_bulk_rule(rule)
